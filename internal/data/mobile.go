...
type LockerOrders struct {
	Id                  uint64    `gorm:"column:id;type:bigint UNSIGNED;comment:订单ID;primaryKey;not null;" json:"id"`                                  // 订单ID
	...
	EndTime             time.Time `gorm:"column:end_time;type:datetime;comment:寄存结束时间（取件时间）;default:NULL;" json:"end_time"`                            // 寄存结束时间（取件时间）
