...
const OperationNearbyGetAllLockerPoints = "/api.helloworld.v1.Nearby/GetAllLockerPoints"
...
type NearbyHTTPServer interface {
	...
	// GetAllLockerPoints 获取所有寄存点（不依赖城市）
...
func RegisterNearbyHTTPServer(s *http.Server, srv NearbyHTTPServer) {
	...
	r.GET("/api/locker-points/all", _Nearby_GetAllLockerPoints0_HTTP_Handler(srv))
}
...
func _Nearby_GetAllLockerPoints0_HTTP_Handler(srv NearbyHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in GetAllLockerPointsRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationNearbyGetAllLockerPoints)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.GetAllLockerPoints(ctx, req.(*GetAllLockerPointsRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
...
type NearbyHTTPClient interface {
	...
	GetAllLockerPoints(ctx context.Context, req *GetAllLockerPointsRequest, opts ...http.CallOption) (rsp *SearchLockerPointsInCityReply, err error)
...
func (c *NearbyHTTPClientImpl) GetAllLockerPoints(ctx context.Context, in *GetAllLockerPointsRequest, opts ...http.CallOption) (*SearchLockerPointsInCityReply, error) {
	var out SearchLockerPointsInCityReply
	pattern := "/api/locker-points/all"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationNearbyGetAllLockerPoints))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
